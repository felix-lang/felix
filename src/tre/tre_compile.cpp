  ...
  while (tre_stack_num_objects(stack) > bottom)
    {
      ...
      symbol = (tre_addtags_symbol_t)(FLX_RAWADDRESS)tre_stack_pop(stack);
      switch (symbol)
        {

        case ADDTAGS_SET_SUBMATCH_END:
          {
            int id = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
            int i;
            ...
            case CATENATION:
              {
                ...
                tre_ast_node_t *left = (tre_ast_node_t*)cat->left;
                tre_ast_node_t *right = (tre_ast_node_t*)cat->right;
                int reserved_tag = -1;
            ...
            case UNION:
              {
                ...
                tre_ast_node_t *left = (tre_ast_node_t*)uni->left;
                tre_ast_node_t *right = (tre_ast_node_t*)uni->right;
                int left_tag;
                int right_tag;
            ...
            if (first_pass)
              {
                node->num_tags = ((tre_iteration_t *)node->obj)->arg->num_tags
                  + (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
                minimal_tag = -1;
              }
            else
              {
                minimal = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
                enter_tag = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
                if (minimal)
        ...
        case ADDTAGS_AFTER_CAT_LEFT:
          {
            int new_tag = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
            next_tag = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
        ...
        case ADDTAGS_AFTER_UNION_RIGHT:
          {
            ...
            tre_ast_node_t *left = (tre_ast_node_t*)tre_stack_pop(stack);
            tre_ast_node_t *right = (tre_ast_node_t*)tre_stack_pop(stack);
            DPRINT(("After union right\n"));
            node = (tre_ast_node_t*)tre_stack_pop(stack);
            added_tags = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
            if (first_pass)
              {
            ...
            tag_left = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
            tag_right = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
  ...
  while (status == REG_OK && tre_stack_num_objects(stack) > bottom)
    {
      ...
      symbol = (tre_copyast_symbol_t)(FLX_RAWADDRESS)tre_stack_pop(stack);
      switch (symbol)
        {
        case COPY_SET_RESULT_PTR:
          result = (tre_ast_node_t**)tre_stack_pop(stack);
          break;
        case COPY_RECURSE:
          node = (tre_ast_node_t*)tre_stack_pop(stack);
          switch (node->type)
            ...
            case CATENATION:
              {
                ...
                *result = tre_ast_new_catenation(mem, cat->left, cat->right);
                ...
                copy = (tre_catenation_t*)(*result)->obj;
                copy->left = NULL;
                copy->right = NULL;
                result = &copy->left;
  ...
  while (status == REG_OK && tre_stack_num_objects(stack) > bottom)
    {
      ...
      symbol = (tre_expand_ast_symbol_t)(FLX_RAWADDRESS)tre_stack_pop(stack);
      node = (tre_ast_node_t*)tre_stack_pop(stack);
      switch (symbol)
        {
        case EXPAND_RECURSE:
        ...
        case EXPAND_AFTER_ITER:
          {
            tre_iteration_t *iter = (tre_iteration_t*)node->obj;
            int pos_add_last;
            pos_add = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
            ...
            if (iter->params)
              {
                ...
                if (!tmp_r)
                  return REG_ESPACE;
                old_params = (unsigned int*)tre_mem_alloc(mem, sizeof(*old_params)
                                           * TRE_PARAM_LAST);
                if (!old_params)
                  return REG_ESPACE;
                for (i = 0; i < TRE_PARAM_LAST; i++)
...
tre_set_one(tre_mem_t mem, int position, int code_min, int code_max,
            tre_ctype_t klass, tre_ctype_t *neg_klasses, int backref)
{
  tre_pos_and_tags_t *new_set;
  ...
  new_set[0].klass = klass;
  new_set[0].neg_klasses = neg_klasses;
  ...
  for (s1 = 0; set1[s1].position >= 0; s1++)
    {
      ...
      new_set[s1].klass = set1[s1].klass;
      new_set[s1].neg_klasses = set1[s1].neg_klasses;
  ...
  for (s2 = 0; set2[s2].position >= 0; s2++)
    {
      ...
      new_set[s1 + s2].klass = set2[s2].klass;
      new_set[s1 + s2].neg_klasses = set2[s2].neg_klasses;
  ...
  while (tre_stack_num_objects(stack) > bottom)
    {
      ...
      symbol = (tre_nfl_stack_symbol_t) (FLX_RAWADDRESS)tre_stack_pop(stack);
      node = (tre_ast_node_t*)tre_stack_pop(stack);
      switch (symbol)
        {
        case NFL_RECURSE:
        ...
        while (p2->position >= 0)
          {
            ...
            trans->assertions = p1->assertions | p2->assertions
              | (p1->klass ? ASSERT_CHAR_CLASS : 0)
              | (p1->neg_klasses != NULL ? ASSERT_CHAR_CLASS_NEG : 0);
            if (p1->backref >= 0)
              {
              ...
              trans->u.klass = p1->klass;
            if (p1->neg_klasses != NULL)
              {
                for (i = 0; p1->neg_klasses[i] != (tre_ctype_t)0; i++);
                trans->neg_klasses =
                  (tre_ctype_t*)xmalloc(sizeof(*trans->neg_klasses) * (i + 1));
                if (trans->neg_klasses == NULL)
                  return REG_ESPACE;
                for (i = 0; p1->neg_klasses[i] != (tre_ctype_t)0; i++)
                  trans->neg_klasses[i] = p1->neg_klasses[i];
                trans->neg_klasses[i] = (tre_ctype_t)0;
              }
            else
              trans->neg_klasses = NULL;
...
#ifdef TRE_DEBUG
            {
              ...
              else if (trans->u.klass)
                DPRINT((", klass %ld", (long)trans->u.klass));
              if (trans->neg_klasses)
                DPRINT((", neg_klasses %p", trans->neg_klasses));
              if (trans->params)
                {
                  DPRINT((", "));
...
tre_compile(regex_t *preg, const tre_char_t *regex, size_t n, int cflags)
{
  ...
  if (tnfa->have_backrefs || !(cflags & REG_NOSUB))
    {
      ...
      tnfa->minimal_tags = (int*)xcalloc(tnfa->num_tags * 2 + 1,
                                   sizeof(tnfa->minimal_tags));
      if (tnfa->minimal_tags == NULL)
        ERROR_EXIT(REG_ESPACE);

      submatch_data = (tre_submatch_data_t*)xcalloc(parse_ctx.submatch_id, sizeof(*submatch_data));
  ...
  transitions = (tre_tnfa_transition_t*)xcalloc(add + 1, sizeof(*transitions));
  if (transitions == NULL)
    ERROR_EXIT(REG_ESPACE);
...
#define TRE_OPTIMIZE_FIRST_CHAR 1
  ...
  initial = (tre_tnfa_transition_t*)xcalloc(i + 1, sizeof(tre_tnfa_transition_t));
  if (initial == NULL)
    ERROR_EXIT(REG_ESPACE);
    ...
    if (tnfa->transitions[i].state)
      {
        ...
        if (tnfa->transitions[i].neg_klasses)
          xfree(tnfa->transitions[i].neg_klasses);
...
#ifdef _WIN32
      _snprintf(str, 256, "TRE %s (GPL)", version);
#else
      snprintf(str, 256, "TRE %s (GPL)", version);
#endif
    }
  return str;
}
