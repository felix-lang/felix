...
typedef enum {
  ...
  REG_ECTYPE,           /* Unknown character klass name. */
...
TRE_EXTERN int regexec(const regex_t *preg, const char *string, size_t nmatch,
            regmatch_t pmatch[], int eflags);
TRE_EXTERN size_t regerror(int errcode, const regex_t *preg, char *errbuf,
                size_t errbuf_size);
TRE_EXTERN void regfree(regex_t *preg);
...
TRE_EXTERN int regncomp(regex_t *preg, const char *regex, size_t len,
  int cflags);
TRE_EXTERN int regnexec(const regex_t *preg, const char *string, size_t len,
             size_t nmatch, regmatch_t pmatch[], int eflags);
#ifdef TRE_WCHAR
TRE_EXTERN int regwncomp(regex_t *preg, const wchar_t *regex, size_t len,
  int cflags);
TRE_EXTERN int regwnexec(const regex_t *preg, const wchar_t *string, size_t len,
              size_t nmatch, regmatch_t pmatch[], int eflags);
#endif /* TRE_WCHAR */
...
TRE_EXTERN char *tre_version(void);
...
/* Returns the value for a config parameter.  The type to which `result'
   ...
   more details. */
TRE_EXTERN int tre_config(int query, void *result);
...
TRE_EXTERN int tre_have_backrefs(const regex_t *preg);

/* Returns 1 if the compiled pattern uses approximate matching features,
   0 if not. */
TRE_EXTERN int tre_have_approx(const regex_t *preg);
