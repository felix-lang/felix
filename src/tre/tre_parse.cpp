...
#else /* !TRE_WCHAR */
      match = strncmp(tre_macros[i], (const char*)regex, strlen(tre_macros[i]));
#endif /* !TRE_WCHAR */
      if (match == 0)
        {
          unsigned int j;
...
/* Expands a character klass to character ranges. */
static reg_errcode_t
tre_expand_ctype(tre_mem_t mem, tre_ctype_t klass, tre_ast_node_t ***items,
                 int *i, int *max_i, int cflags)
{
  ...
  DPRINT(("  expanding klass to character ranges\n"));
  for (j = 0; (j < 256) && (status == REG_OK); j++)
    {
      c = j;
      if (tre_isctype(c, klass)
          || ((cflags & REG_ICASE)
              && (tre_isctype(tre_tolower(c), klass)
                  || tre_isctype(tre_toupper(c), klass))))
{
          if (min < 0)
            min = c;
          max = c;
...
/* Maximum number of character klasses that can occur in a negated bracket
   expression.  */
#define MAX_NEG_CLASSES 64

/* Maximum length of character klass names. */
#define MAX_CLASS_NAME

static reg_errcode_t
tre_parse_bracket_items(tre_parse_ctx_t *ctx, int negate,
                        tre_ctype_t neg_klasses[], int *num_neg_klasses,
                        tre_ast_node_t ***items, int *num_items,
                        int *items_size)
{
  const tre_char_t *re = ctx->re;
  reg_errcode_t status = REG_OK;
  tre_ctype_t klass = (tre_ctype_t)0;
  int i = *num_items;
      ...
      else
        {
          ...
          if (re + 2 < ctx->re_end
                   ...
                   && *re == CHAR_LBRACKET && *(re + 1) == CHAR_COLON)
            {
              ...
              int len;
              DPRINT(("tre_parse_bracket:  klass: '%.*" STRF "'\n",
                      ctx->re_end - re, re));
              ...
              if (endptr != ctx->re_end)
                {
                  ...
                  tmp_str[len] = '\0';
                  DPRINT(("  klass name: %s\n", tmp_str));
                  klass = tre_ctype(tmp_str);
                  if (!klass)
                    status = REG_ECTYPE;
                  /* Optimize character klasses for 8 bit character sets. */
                  if (status == REG_OK && TRE_MB_CUR_MAX == 1)
                    {
                      status = tre_expand_ctype(ctx->mem, klass, items,
                                                &i, &max_i, ctx->cflags);
                      klass = (tre_ctype_t)0;
                      skip = 1;
                    }
          ...
          if (klass && negate)
            if (*num_neg_klasses >= MAX_NEG_CLASSES)
              status = REG_ESPACE;
            else
              neg_klasses[(*num_neg_klasses)++] = klass;
          else if (!skip)
            {
              status = tre_new_item(ctx->mem, min, max, &i, &max_i, items);
              if (status != REG_OK)
                break;
              ((tre_literal_t*)((*items)[i-1])->obj)->u.klass = klass;
            }

          /* Add opposite-case counterpoints if REG_ICASE is present.
             This is broken if there are more than two "same" characters. */
          if (ctx->cflags & REG_ICASE && !klass && status == REG_OK && !skip)
            {
              int cmin, ccurr;
...
tre_parse_bracket(tre_parse_ctx_t *ctx, tre_ast_node_t **result)
{
  ...
  tre_ctype_t neg_klasses[MAX_NEG_CLASSES];
  int num_neg_klasses = 0;
  ...
  status = tre_parse_bracket_items(ctx, negate, neg_klasses, &num_neg_klasses,
                                   &items, &i, &max_i);
  ...
  for (j = 0; j < i && status == REG_OK; j++)
    {
      ...
      DPRINT(("item: %d - %d, klass %ld, curr_max = %d\n",
              (int)l->code_min, (int)l->code_max, (long)l->u.klass, curr_max));
      ...
      if (l != NULL)
        {
          ...
          if (num_neg_klasses > 0)
            {
              l->neg_klasses = (tre_ctype_t*)tre_mem_alloc(ctx->mem,
                                             (sizeof(l->neg_klasses)
                                              * (num_neg_klasses + 1)));
              if (l->neg_klasses == NULL)
                {
                  status = REG_ESPACE;
                  break;
                }
              for (k = 0; k < num_neg_klasses; k++)
                l->neg_klasses[k] = neg_klasses[k];
              l->neg_klasses[k] = (tre_ctype_t)0;
            }
          else
            l->neg_klasses = NULL;
          if (node == NULL)
      ...
      if (approx || costs_set || counts_set)
        {
          ...
          ctx->have_approx = 1;
          params = (unsigned int*)tre_mem_alloc(ctx->mem, sizeof(*params) * TRE_PARAM_LAST);
          if (!params)
            return REG_ESPACE;
  ...
  while (tre_stack_num_objects(stack) > bottom && status == REG_OK)
    {
      if (status != REG_OK)
        break;
      symbol = (tre_parse_re_stack_symbol_t)(FLX_RAWADDRESS)tre_stack_pop(stack);
      switch (symbol)
        {
        case PARSE_RE:
        ...
        case PARSE_POST_CATENATION:
          {
            tre_ast_node_t *tree = (tre_ast_node_t*)tre_stack_pop(stack);
                ...
                if (buf[0] != 0)
                  {
                    ...
                    subctx.re = buf;
                    subctx.len = tre_strlen((const char*)buf);
                    subctx.nofirstsub = 1;
        ...
        case PARSE_MARK_FOR_SUBMATCH:
          {
            int submatch_id = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
        ...
        case PARSE_RESTORE_CFLAGS:
          ctx->cflags = (int)(FLX_RAWADDRESS)tre_stack_pop(stack);
          break;
        }
    }
