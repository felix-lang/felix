                    ...
                    int eflags, int *match_end_ofs)
{
     ...
     or with malloc() if alloca is unavailable. */
  {
    ...
    /* Allocate the memory. */
#ifdef TRE_USE_ALLOCA
    buf = (unsigned char*)alloca(total_bytes);
#else /* !TRE_USE_ALLOCA */
    ...
    reach = (tre_tnfa_approx_reach_t*)(void *)buf_cursor;
    buf_cursor += reach_bytes;
    buf_cursor += ALIGN(buf_cursor, long);

    /* Allocate `reach_next' from `buf'. */
    reach_next = (tre_tnfa_approx_reach_t*)(void *)buf_cursor;
    buf_cursor += reach_bytes;
    buf_cursor += ALIGN(buf_cursor, long);
    ...
    for (i = 0; i < tnfa->num_states; i++)
      {
        reach[i].tags = (int*)(void *)buf_cursor;
        buf_cursor += tag_bytes;
        reach_next[i].tags = (int*)(void *)buf_cursor;
        buf_cursor += tag_bytes;
      }
              ...
              if (trans->assertions
                  && (CHECK_ASSERTIONS(trans->assertions)
                      ...
                      || ((trans->assertions & ASSERT_CHAR_CLASS)
                          && !(cflags & REG_ICASE)
                          && !tre_isctype((tre_cint_t)prev_c, trans->u.klass))
                      || ((trans->assertions & ASSERT_CHAR_CLASS)
                          && (cflags & REG_ICASE)
                      ...
                      || ((trans->assertions & ASSERT_CHAR_CLASS_NEG)
                          && tre_neg_char_klasses_match(trans->neg_klasses,
                                                        (tre_cint_t)prev_c,
                                                        cflags & REG_ICASE))))
                {
