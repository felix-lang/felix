...
inline static int
tre_neg_char_klasses_match(tre_ctype_t *klasses, tre_cint_t wc, int icase)
{
  DPRINT(("neg_char_klasses_test: %p, %d, %d\n", klasses, wc, icase));
  while (*klasses != (tre_ctype_t)0)
    if ((!icase && tre_isctype(wc, *klasses))
        || (icase && (tre_isctype(tre_toupper(wc), *klasses)
                      || tre_isctype(tre_tolower(wc), *klasses))))
      return 1; /* Match. */
    else
      klasses++;
  return 0; /* No match. */
