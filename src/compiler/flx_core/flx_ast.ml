...
let rec typ_to_expr dsr = function
  ...
  | TYP_void (sr) -> Some( EXPR_void (sr) )
  | TYP_name (sr, id, ts) -> Some( EXPR_name (sr, id, ts) )
  | TYP_case_tag (sr, i) -> Some( EXPR_case_tag (sr, i) )
  | TYP_typed_case (sr, i, ts) -> Some( EXPR_typed_case (sr, i, ts) )
  | TYP_lookup (sr, (ex,id,ts)) -> Some( EXPR_lookup (sr, (ex,id,ts)) )
  | TYP_index (sr, name, index) -> Some( EXPR_index (sr, name, index) )
  | TYP_callback (sr, qn) -> Some( EXPR_callback (sr, qn) )
  | TYP_suffix (sr, (qn, t)) -> Some( EXPR_suffix (sr, (qn, t)) )
  | TYP_patvar (sr,id) -> Some( EXPR_patvar (sr,id) )
  | TYP_patany (sr) -> Some( EXPR_patany (sr) )
  | TYP_typeof (e) -> Some( EXPR_typeof (dsr, e) )
  | TYP_ellipsis -> Some( EXPR_ellipsis (dsr) )

  | TYP_as (t, id) -> (
      match (typ_to_expr dsr t) with 
      | Some e -> Some( EXPR_as (dsr, (e, id)) )
      | None -> None)

  | TYP_tuple (ts) -> 
      (* recusively convert the type expressions in ts *)
      let converted = (List.map (typ_to_expr dsr) ts) in

      (* manipulate data for output *)
      let exprs = ref [] in
      let incomplete = ref false in

      (* Loop through converted expressions and aggregate *)
      List.iter (function
          | Some (e) -> exprs := e :: !exprs
          | None -> incomplete := true)
        converted; 

      if !incomplete then None 
      else Some( EXPR_tuple (dsr, !exprs))
  ...
  | TYP_intersect (ts) -> 
      ...
      if !incomplete then None 
      else Some( EXPR_intersect (dsr, !exprs))
  ...
  | TYP_record (ids_and_ts) -> 
      ...
      (* Loop through converted expressions and aggregate *)
      List.iter (function
          | id, Some (e) -> exprs := (id, e) :: !exprs
          | id, None -> incomplete := true)
        converted; 

      if !incomplete then None 
      else Some( EXPR_record (dsr, !exprs))
  ...
  | TYP_polyrecord (ids_and_ts, t2) -> 
      ...
      let converted1 = 
          (List.map 
            ...
            ids_and_ts) 
      in
      let converted2 = (typ_to_expr dsr t2) in
      ...
      if !incomplete then None 
      else (match converted2 with 
        | Some(expr2) -> Some( EXPR_polyrecord (dsr, !exprs, expr2))
        | None -> None)


  | TYP_effector (t1,t2,t3) -> 
      let opt_e1 = (typ_to_expr dsr t1) in
      let opt_e2 = (typ_to_expr dsr t2) in
      let opt_e3 = (typ_to_expr dsr t3) in

      (match (opt_e1, opt_e2, opt_e3) with
      | Some(e1),Some(e2),Some(e3) -> 
          Some( EXPR_effector (dsr, (e1, e2, e3)))
      | _ -> None)
  ...
  | TYP_tuple_cons (sr, t1, t2) -> 
      ...
      (match (opt_e1, opt_e2) with
      | Some(e1),Some(e2) ->
          Some( EXPR_tuple_cons (dsr, e1, e2))
      | _ -> None)
