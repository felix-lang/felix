...
type t =
  ...
  | BBDCL_union of      bvs_t * (Flx_id.t * int * Flx_btype.t * Flx_btype.t) list
...
=
  ...
  | BBDCL_external_code (_,_,_,breqs) ->
      f_breqs breqs
  | BBDCL_union (_,cs) ->
      List.iter (fun (_,_,d,c) -> f_btype d; f_btype c) cs
  | BBDCL_struct (_,cs) ->
...
=
  ...
  | BBDCL_union (bvs,cs) ->
      BBDCL_union (bvs,List.map (fun (n,i,d,c) -> n,i,f_btype d,f_btype c) cs)
  | BBDCL_struct (bvs,cs) ->
