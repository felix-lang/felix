...
let rec print f = function
  ...
  | BBDCL_val (bvs,t,kind) ->
      let print_kind f = function
        | `Val -> print_variant0 f "`Val"
        | `Var -> print_variant0 f "`Var"
        | `Ref -> print_variant0 f "`Ref"
        | `Tmp -> print_variant0 f "`Tmp"
      in
      print_variant3 f "BBDCL_val"
        print_bvs bvs
        Flx_btype.print t
        print_kind kind
  | BBDCL_newtype (bvs,t) ->
      print_variant2 f "BBDCL_newtype" print_bvs bvs Flx_btype.print t
  | BBDCL_external_type (bvs,quals,code,reqs) ->
  ...
  | BBDCL_external_fun (props,bvs,ps,rt,reqs,prec,kind) ->
      ...
      print_variant7 f "BBDCL_external_fun"
        ...
        print_string prec
        print_kind kind
  | BBDCL_external_code (bvs,code,ikind,reqs) ->
      let ikind =
        match ikind with
        | `Header -> "header"
        | `Body -> "body"
