...
*)
    ...
    | `EXE_apply_ctor (vname, clsname, arg) ->
      let (e2,t2) as barg = be arg in
      let var_idx =
        let varname = `AST_name (sr,vname,[]) in
        match be varname with
          | `BEXPR_name (i,_),_ -> i
          | _ -> clierr sr "Expected variable name to store object"
      in
      let cls = be clsname in
      begin match cls with

      | `BEXPR_name (class_idx,ts),_ ->
        begin
          match
            try hfind "bexe" syms.dfns class_idx
            with Not_found ->
              syserr sr ("[bexe][EXE_apply_ctor] Weird, can't find class index " ^ si class_idx)
          with
          | {id=name;pubmap=pubmap;vs=class_vs; symdef=`SYMDEF_class} ->
            print_endline ("Found a class "^name^
              ", vs="^catmap "," (fun (s,i,_)->s) (fst class_vs)
              ^" look for constructor with hacked name _ctor_"^name);
            ...
            | Some (`NonFunctionEntry _) -> syserr sr
              "[EXE_apply_ctor: lookup_name_in_table_dirs_with_sig] Expected constructor to be a procedure"
            ...
            | Some (`FunctionEntry fs) ->
              ...
              *)
              (*
              The constructors are specified as non-polymorphic, but we need the overload
              to include the class type variables, so we hack it up by pre-pending the
              class type variables to the constructor views
              *)
              let fs = map (fun ({base_sym=i; spec_vs=vs; sub_ts=ts} as view) ->
                (* these failure shouldn't happen by construction .. *)
                if length vs <> 0 then
                  failwith "Unexpected polymorphic view of class constructor!"
                ;
                if length ts <> 0 then
                  failwith "Unexpected polymorphic class constructor!"
                ;
                {view with spec_vs=map (fun (s,i,_)->s,i) (fst class_vs) }
                ) fs
              in
              let ro =
                resolve_overload
                syms env sr fs ("_ctor_" ^ name) [t2] ts
              in
              match ro with
                | Some (ctor_idx,t,ret,mgu,ts') ->
                  (* The overload resolution is generic, but the application
                    is concrete. so ts' should be a list of type variables
                    corresponding to the class vs, and the mgu should
                    map these to the ts used to instantiate the class..??
                  *)
                  if length ts' <> length ts then
                    clierr sr ("[EXE_apply_ctor] Type subscript mismatch:\n" ^
                    "got type subscripts " ^ catmap "," (sbt syms.dfns) ts')
                  ;
                  assert (length (fst class_vs) = length ts');
                  (* NOTE: we should check the type constraint here! i.e. the aux_vs thing *)
                  tack (`BEXE_apply_ctor (sr,var_idx,class_idx,ts,ctor_idx, barg))
                | None ->
                  clierr sr
                  (
                    "Unable to find matching constructor for class " ^ name ^
                    "<" ^ si class_idx ^ ">[" ^
                    catmap "," (sbt syms.dfns) ts ^ "](" ^
                    sbt syms.dfns t2 ^ "),\n" ^
                    "Candidates were:\n" ^ catmap "\n" (full_string_of_entry_kind syms.dfns) fs
                  )
            end
          | _ -> clierr sr "Argument of new must be a class"
        end
      | `BEXPR_closure (i,ts),_ ->
        clierr sr ("Class constructor must name class, and we got a closure (which is right but unexpected ..)")

      | _ ->
        clierr sr ("Class constructor must name class, got " ^ sbe syms.dfns bbdfns cls)
      end
