...
(*let grammar =
  ...
  let aux (new_gr,newnt_nb) ra =
    ...
    let lhs_nt,prio,ld_list,par_act_l,
      (ocaml_code,ocaml_code_pos), rule_options = ra in
    if par_act_l = [] then
      ((lhs_nt,prio,ld_list,par_act_l,
        ((ocaml_code,("","")),ocaml_code_pos),rule_options)
      ::new_gr,newnt_nb)
    else
    ...
    let rec f new_gr newnt_nb litl last_pos res_nb
      ...
      | (((ac_code,i),pos),patl)::tl ->
          ...
          let new_nt = if res_nb = 0 then [] else
            [(Symb_non_terminal
              (("dypgen__nt_"^(string_of_int (newnt_nb-1)),dummy_p),
            ("No_priority",dummy_p),Pr_eq,res_nb,dummy_code)),patternl]
          in
          ...
          let new_gr =
            (("dypgen__nt_"^(string_of_int newnt_nb),("_",Lexing.dummy_pos)),
            ("default_priority",dummy_p),
            new_litl,[],(ac_code,i),rule_options)::new_gr
          in
    ...
    let new_gr =
      (lhs_nt,prio,new_litl,[],
      ((ocaml_code,("","")),ocaml_code_pos),rule_options)::new_gr
    in
    (new_gr,newnt_nb)
  in
  let g,_ = List.fold_left aux ([],0) grammar in
  g*)
...
(*let grammar =
  let aux ra =
    let lhs, p, symb_list, pal, action, ro = ra in
    match symb_list with
    | ((Symb_non_terminal ((nt,_),_,_,_,_)),_)::_
    | ((Symb_non_terminal_NL ((nt,_),_,_,_,_)),_)::_
      when String_set.mem nt nt_par_set ->
        let dypgen_epsilon =
          (Symb_non_terminal (("dypgen__epsilon",dummy_p),
          ("No_priority",dummy_p),Pr_eq,1,dummy_code)),
          ["_","dypgen__epsilon",Lexing.dummy_pos]
        in
        lhs, p, dypgen_epsilon::symb_list, pal, action, ro
    | _ -> ra
  in
  List.map aux grammar*)
  ...
  let l2 = List.map
    ...
    let c =
      if typ = "No_type" then
        ...
        (Printf.fprintf stderr "Error: line %d, an action is expected for terminal %s\n" pos.pos_lnum tname; exit 2)
...
let code_parser =
  ...
  ocaml_code, (allow_layout_inside, allow_layout_follows)) =
    ...
    let code_var_list, code_inherited_val =
              ...
              let typ =
                ...
                (match pat_typ with
                  | Pat_syn pt ->
                      ...
                      ":'dypgen__"^(try String_map.find pt nt_type_map
                      with Not_found -> make_type_var_aux pt)
                  | Pat_inh pt -> ":'dypgen__"^
      ...
      in
      let code_vl, code_iv, _, _ = List.fold_left f (code_vl,[],1,1) symb_list in
