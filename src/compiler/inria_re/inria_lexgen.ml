...
let string_of_automata_move x = match x with
  | Backtrack -> "backtrack"
  | Goto i -> "goto " ^ string_of_int i

let string_of_tag_base x = match x with
  | Start -> "Start"
  | Mem j -> "Mem " ^ string_of_int j
  | End -> "End"

let string_of_tag_addr x = match x with Sum (tb,i) ->
  string_of_tag_base tb ^ " " ^ string_of_int i

let string_of_ident_info x = match x with
  | Ident_string (b,t1,t2) ->
    "string " ^
    if b then "true " else "false " ^
   "(" ^ string_of_tag_addr t1 ^ ", " ^ string_of_tag_addr t2 ^ ")"
 | Ident_char (b,t1) ->
    "char " ^
    if b then "true " else "false " ^
   "(" ^ string_of_tag_addr t1 ^ ")"

let string_of_tenv iidis =
  if List.length iidis = 0 then "none" else
  (
  String.concat ";\n"
    (
      List.map
      (fun (s,idi) -> "  var " ^ s ^ ":" ^string_of_ident_info idi)
      iidis
    )
  ) ^ "\n"

(* Representation of entry points *)
let string_of_tag_action x = match x with
  | SetTag (i,j) -> "set " ^ string_of_int i ^ " " ^ string_of_int j
  | EraseTag i -> "erase " ^ string_of_int i

let string_of_memory_action x = match x with
  | Copy(i,j) -> "copy " ^ string_of_int i ^ " " ^ string_of_int j
  | Set i -> "set " ^ string_of_int i

let string_of_tag_action_list x =
  String.concat ", " (List.map string_of_tag_action x)

let string_of_automata_trans x = match x with
  | No_remember -> "No_remember"
  | Remember  (i,tal) -> "Remember " ^ string_of_int i ^ " "^
    string_of_tag_action_list tal

let string_of_automata a = match a with
  | Perform (i,tal) -> "perform " ^ string_of_int i ^ " " ^
    string_of_tag_action_list tal

  | Shift (at,vect) -> "shift " ^ string_of_automata_trans at ^
    "\n" ^
    (
      let s = ref "" in
      for i = 0 to Array.length vect - 1 do
        let move,macts = vect.(i) in
        if move=Backtrack && List.length macts = 0 then () else
        s:= !s ^ "Char " ^ string_of_int i ^ " " ^
          "move="^ string_of_automata_move move ^
          ", macts= " ^
          (
            if List.length macts = 0 then "none" else
            String.concat ", "
            (List.map string_of_memory_action macts)
          ) ^ "\n"
      done;
      !s
    )
...
let string_of_automata_entry {
    auto_name = auto_name;
    auto_mem_size = size ;
    auto_initial_state = (i,mal);
    auto_actions = itenvas
} =
  auto_name ^ " size=" ^ string_of_int size ^
  ", init_state=" ^ string_of_int i ^ "\nmemacts=" ^
  (if List.length mal = 0 then "none" else
  String.concat "," (List.map (fun ma -> string_of_memory_action ma) mal)
  ) ^ "\n" ^
  "actions=\n" ^
  String.concat "" (List.map (fun (i,te,_) ->
    "action " ^ string_of_int i ^ "\n  " ^ string_of_tenv te
  ) itenvas)
...
let chars = ref ([] : Inria_cset.t list)
...
let rec encode_regexp char_vars act = function
  ...
  | Characters cl ->
      ...
      chars := cl :: !chars;
      incr chars_count;
      print_endline ("Position " ^ string_of_int n ^ "->" ^ Inria_cset.string_of_characters cl);
      Chars(n,false)
  | Eof ->
      let n = !chars_count in
      chars := Inria_cset.eof :: !chars;
      incr chars_count;
      Chars(n,true)
  | Sequence(r1,r2) ->
...
let state_table = Inria_table.create dfa_state_empty
...
let reset_state_mem () =
  ...
  Stack.clear todo;
  next_state_num := 0 ;
  let _ = Inria_table.trim state_table in
  ()
...
let get_state st =
  ...
  try
    ...
    num,move_to key.kmem st (Inria_table.get state_table num)
  with Not_found ->
    let num = !next_state_num in
    incr next_state_num;
    let st,mvs = create_new_state st in
    Inria_table.emit state_table st ;
...
let rec split_env gen follow pos m s = function
  | [] -> (* Can occur ! because of non-matching regexp ([^'\000'-'\255']) *)
      []
  | (s1,st1) as p::rem ->
      let here = Inria_cset.inter s s1 in
      if Inria_cset.is_empty here then
        p::split_env gen follow pos m s rem
      else
        let rest = Inria_cset.diff s here in
        let rem =
          if Inria_cset.is_empty rest then
            rem
          else
            split_env gen follow pos m rest rem
        and new_st = apply_transitions gen st1 pos m follow in
        let stay = Inria_cset.diff s1 here in
        if Inria_cset.is_empty stay then
          (here, new_st)::rem
        else
...
let comp_shift gen chars follow st =
  MemMap.fold
    ...
    st [Inria_cset.all_chars_eof,dfa_state_empty]
...
(* finally build the char indexed array -> new state num *)
  let shift = Inria_cset.env_to_array env in
  shift
...
(* *)
  ...
  for i = 0 to !next_state_num-1 do
    eprintf "+++ %d +++\n" i ;
    dstate (Inria_table.get state_table i) ;
    prerr_endline ""
  done ;
  eprintf "%d states\n" !next_state_num ;
(* *)
  ...
  for i = 0 to !next_state_num - 1 do
    let act = actions.(i) in
    print_endline
    ("State " ^ string_of_int i ^ " has action " ^ string_of_automata act)
  done;
  reset_state_mem () ;
  reset_cell_mem  0 ;
  ...
  List.iter
  (fun ae ->
    print_endline "--- automata entry -- ";
    print_endline (string_of_automata_entry ae);
  )
  initial_states
  ;
