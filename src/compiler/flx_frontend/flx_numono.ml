...
let flat_typeclass_fixup_type syms bsym_table fi t =
  match t with
  | BTYP_inst (i,ts) ->
    let i',ts' = fi i ts in
...
let rec typeclass_fixup_type syms bsym_table fi t =
  let f_btype t = typeclass_fixup_type syms bsym_table fi t in
  let t = Flx_btype.map ~f_btype t in
  let t = flat_typeclass_fixup_type syms bsym_table fi t in
  t

let flat_typeclass_fixup_expr syms bsym_table fi (e,t) =
  let x = match e with
...
let rec typeclass_fixup_expr syms bsym_table fi e =
  let f_bexpr e = typeclass_fixup_expr  syms bsym_table fi e in
  let e = Flx_bexpr.map ~f_bexpr e in
  let e = flat_typeclass_fixup_expr syms bsym_table fi e in
  e 

(* mt is only used to fixup svc and init hacks *)
let flat_typeclass_fixup_exe syms bsym_table fi mt exe =
(*
...
let flat_poly_fixup_type syms bsym_table fi t =
  match t with
  | BTYP_inst (i,ts) ->
    let i',ts' = fi i ts in
    let t = btyp_inst (i',ts') in
...
let rec poly_fixup_type syms bsym_table fi t =
  let f_btype t = poly_fixup_type syms bsym_table fi t in
  let t = Flx_btype.map ~f_btype t in
  let t = flat_poly_fixup_type syms bsym_table fi t in
  t

let flat_poly_fixup_expr syms bsym_table fi (e,t) =
  let x = match e with
  | BEXPR_apply_prim (i',ts,a) -> assert false
  | BEXPR_apply_direct (i,ts,a) -> assert false
  | BEXPR_apply_struct (i,ts,a) -> assert false
...
let rec poly_fixup_expr syms bsym_table fi e =
  let f_bexpr e = typeclass_fixup_expr  syms bsym_table fi e in
  let e = Flx_bexpr.map ~f_bexpr e in
  let e = flat_typeclass_fixup_expr syms bsym_table fi e in
  e 
...
(* mt is only used to fixup svc and init hacks *)
let flat_poly_fixup_exe syms bsym_table fi mt exe =
(*
  print_endline ("TYPECLASS FIXUP EXE[In] =" ^ string_of_bexe bsym_table 0 exe);
*)
  let result =
...
let fixup_type syms bsym_table vars bsym virtualinst fi t =
(*
...
*)
  let nut2 = typeclass_fixup_type syms bsym_table virtualinst  nut1 in
  let nut3 = poly_fixup_type syms bsym_table fi nut2 in
...
let fixup_req syms bsym_table vars fi (i,ts) : Flx_types.bid_t * Flx_btype.t list =
  let ts = List.map (mono_type syms bsym_table vars) ts in
  let j,ts' = fi i ts in
  j,ts'

let fixup_reqs syms bsym_table vars fi reqs : Flx_bbdcl.breqs_t = 
  List.map (fixup_req syms bsym_table vars fi) reqs
...
let fixup_expr syms bsym_table monotype virtualinst fi e =
  ...
  *)
  let e = poly_fixup_expr syms bsym_table fi e in
  e
...
*)
  (* replace applications of polymorphic function (or variable)
    with applications of new monomorphic ones
  *)
  let exes = List.map (fun exe -> Flx_bexe.map ~f_bexpr:(poly_fixup_expr syms bsym_table fi) exe) exes in
(*
  print_endline ("Applies monomorphised:\n" ^ show_exes bsym_table exes);
*)
  (* replace weird cases in exes with implied ts *)
  let exes = List.map (flat_poly_fixup_exe syms bsym_table fi mt)  exes in
(*
...
*)
  ...
  let mt vars t = fixup_type syms bsym_table vars bsym virtualinst fi t in
  ...
  *)
  | BBDCL_external_fun (props,vs,argtypes,ret,reqs,prec, fkind) ->
    ...
    let ret = mt vars ret in
    let reqs = fixup_reqs syms bsym_table vars fi reqs in
  ...
  | BBDCL_union (vs,cps) -> 
    assert (List.length vs = List.length ts);
    let vars = List.map2 (fun (s,i) t -> i,t) vs ts in
    let cpts = List.map (fun (name,index,argt) -> name,index, mt vars argt) cps in
    Some (bbdcl_union ([], cps))

  | BBDCL_cstruct (vs,cps, breqs) -> 
    assert (List.length vs = List.length ts);
    let vars = List.map2 (fun (s,i) t -> i,t) vs ts in
    let cpts = List.map (fun (name,argt) -> name,mt vars argt) cps in
    Some (bbdcl_cstruct ([], cps, breqs))
