    ...
    | [ut] ->
      ...
      | BTYP_inst (index, ts) ->
        let bsym = 
           ...
           with Not_found -> syserr sr ("Can't find index " ^ si index ^ " in bound symbol table")
        in
(*
        print_endline ("Found index " ^ si index ^ " -> " ^ Flx_bsym.id bsym);
*)
        let bbdcl = Flx_bsym.bbdcl bsym in
        let open Flx_bexe in
        let open Flx_bexpr in
        begin (* 8*)match bbdcl with
        | BBDCL_union (ubvs, flds)  -> 
          let uname = Flx_bsym.id bsym in
          print_endline ("Generating union decoder chip _decoder_" ^ uname); 
          let ischannel_t = bt sr (TYP_name (sr,"ischannel", [ut])) in
          let oschannel_t ubt = bt sr (TYP_name (sr,"oschannel",[ubt])) in
(*
...
*)
          ...
          let ubt t = Flx_typecode_of_btype.typecode_of_btype bsym_table state.counter sr t in
...
*)
          let outer_fun_index = Flx_bid.fresh_bid state.counter in
          let inner_proc_index = Flx_bid.fresh_bid state.counter in

          (* construct parameter *)
          let param_index = Flx_bid.fresh_bid state.counter in
          let param_type = btyp_record ((uname,ischannel_t) :: ochans) in
          let param_name = "io" in
          let param = {  pid = param_name; pindex = param_index; pkind = `PVal; ptyp= param_type }  in

          let props = [] in
          let bvs = [] in
          let effects = btyp_unit () in
          let inner_proc_ret = Flx_btype.btyp_function ( Flx_btype.btyp_unit (), Flx_btype.btyp_void () ) in
          let label_index = Flx_bid.fresh_bid state.counter in
          let param_var = bexpr_varname param_type (param_index,[]) in
          (* read input channel *)
          let read_var_index = Flx_bid.fresh_bid state.counter in
...
*)
            let next_label_index = Flx_bid.fresh_bid state.counter in
            let field_name, field_t = List.nth flds k in
            let ochan_t = oschannel_t (Flx_typecode_of_btype.typecode_of_btype bsym_table state.counter sr field_t) in
            ...
            let next_label_bbdcl = bbdcl_label next_label_name in 
          ...
          let match_body =  List.concat (List.map case (nlist (List.length flds))) in
...
*)
          ...
          let read_var_bbdcl = bbdcl_val ([], but, `Val) in 
          ...
          (* add label to symbol table *)
          let label_bbdcl = bbdcl_label ("_start") in 
          ...
          let inner_proc_bbdcl = bbdcl_fun (props, bvs, ([], None), btyp_void (), effects, exes) in
          ...
          let param_bbdcl = bbdcl_val ([], param_type, `Val) in 
          ...
          let outer_fun_bbdcl = bbdcl_fun (props, bvs, ([param], None), unit_proc_type, effects, outer_fun_exes) in
          ...
          bexpr_closure outer_fun_type (outer_fun_index,[])

        | _ -> clierr2 sr (Flx_bsym.sr bsym) ("_decoder requires union argument, got " ^ Flx_bsym.id bsym);
        end (*8 BTYP_inst*)
      | _ -> clierr sr ("_decoder type argument must be union instance")
      end (*6 match but *)
    | _ -> clierr sr ("_decoder requires single type argument")
    end (*4 match ts *)
