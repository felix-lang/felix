...
*)
  let bt sr t =
    ...
    *)
    let t = inner_bind_type "lookup_qn_with_sig'" state bsym_table env sr rs t in
    let t = Flx_beta.beta_reduce "[lookup_qn_with_sig]" state.counter bsym_table sra t in
    t
  in
