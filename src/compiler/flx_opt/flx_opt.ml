...
let optimize_bsym_table' syms bsym_table (root_proc: int option) =
  ...
  let bsym_table = print_time syms "[flx_opt]; Expanding Coercions (early)" begin fun () ->
  (* make wrappers for non-function functional values *)
  Flx_xcoerce.expand_coercions syms bsym_table end
  in
...
*)
  ...
  print_time syms "[flx_opt]; Very Late Shared variable initialisation Verification" begin fun () ->
  Flx_shareinit.shareinit_bsym_table "Monomorphic" bsym_table syms.counter end;
