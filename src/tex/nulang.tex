...
Nevertheless for a language targetting some particular VM the following plan is proposed.
\begin{itemize}
...
The front end of the compiler should be as generic (not specific to a particular VM) 
as possible because now the language itself becomes the defining standard, abstracting away
the back end VM. This then allows the VM to be modified to improve performance
without impacting the semantics of existing programs.

\section{Non-determinism}
Some VM system uses two languages: the core VM assermbler language is used for deterministic
computations which can be required to generate proofs of execution. However some have 
a feature where an "oracle" can "guess" the result of computation, and the system
merely checks that the guess is correct. 
