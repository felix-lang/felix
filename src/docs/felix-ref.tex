...
\begin{minted}{felix}
  double * double -> double        // type of RHS projection function
  &(double * double) -> &double    // type of LHS projection function
\end{minted}
...
\begin{center}
{\em Pointers provide the bridge
...
Spawns a new pthread\gls{pthread} which executes the argument procedure.
...
of more than one view of the same function, then they must map to the same
...
\section{Basic {\tt type} statement}\index{type!statement}
...
\subsection{{\tt _gc_pointer} types}
...
\section{Function bindings}\index{binding!function}
...
\section{Conversion bindings}\index{binding!function}
...
file or \verb%body% file Felix generates.
...
so that a requires clause may specify the name
...
Insertions can also be polymorphic.
...
The effect is to apply the requirement to all bindings
...
the flx_pkgconfig system,  mapping the abstract
...
\chapter{Algebraic Structure of numeric types}
