...
\section{Unqualified name lookup}
...
type equality of corresponding index expressions
using a unification algorithm. For example
...
it aborts the compiler with a diagnostic message.
...
discovered, if not it aborts the compiler with a 
diagnostic.

\subsection{Basic unqualified function lookup}
When seeking a function, felix follows similar rules
to non-functions, except as follows. When a function
symbol is found, it consists of a set of functions,
