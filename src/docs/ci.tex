...
With integrated linear stacks, the serious deficiencies of the machinery
only exhibit with more complex problems in which a subroutine is repeatedly
called but requires state calculated from the last call. This is 
evident with message handling callbacks in GUI systems, and functional
programming using higher order functions.

Our working example of the serious deficiencies of the subroutine
call model will be a functional fold and a equivalent 
procedural iteration, but first we must study an important
code refactoring technique known as {\em control inversion}.
...
Let us begin with the simple situation illustrated above, in which
a master and three slaves are used to effect a fixed sequence
of effects. We will show how to {\em control invert} the subroutine
call to \verb%slave2% so that it becomes the master and the 
master a slave:
...
What we have done here is as follows: the new master
must first perform all the tasks that the master previously
performed up to the point where it was called. Then it does
its own work, and finally, all the work the master would have
done after it returned.

Let us study a slighly more difficult problem, where the utility
of this procedure becomes a little more evident.
...
The slave routine here illustrate very bad programming style:
it saves essential state in global variables and hence is 
not re-entrant. But there is no alternative whilst the master
dispatcher insists on calling it without an auxilliary state
