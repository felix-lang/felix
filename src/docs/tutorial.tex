...
represents set $D$, type \verb%C% represents set $D$, then for all elements
...
The final, outer call of a function is known as a tail
call. In expanded form, in this function for example:
...
Although this is only a small optimisation, if the tail call is to
the function itself, we can make another improvement. Instead of
pushing the argument onto the stack, we can just assign the
