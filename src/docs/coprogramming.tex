...
and we can define the option monad:
...
\begin{minted}{felix}
//Override the right shift assignment operator.
syntax monad 
{
  x[ssetunion_pri] := 
...
var r : opt[D] = ret a >>= f >>= g >>= h;
\end{minted}

Here, the \verb%ret% is a lift of a plain value to the monad type,
we would do a match to drop the monadic value r back.
A partial function \verb%A->B% can be lifted to a total function
\verb%A->opt[B]% by returning an option type. 

But there is another way:

\begin{minted}[bgcolor=felixbg]{felix}
var r = None[D];
run (a.value |-> f.filter |-> g.filter |-> h.filter 
