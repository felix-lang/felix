...
\section{Duality of products and sums}
THIS BIT IS TOO WAFFLY.

We can also define our original decoder using a duplicator
and two projections:
...
and now we just connect the two projection filters to the two channels.
The \verb%dup% chip is also known as the diagonal operator 
and written in greek as $\Delta$. In imperative programming, it is 
called \verb%copy% or \verb%clone%.

There is a dual chip named \verb%merge% which reads two channels
of the same type, and outputs the result on a single channel,
the mathematical name is $\nabla$ which pronounced Nabla,
is an upside down delta, it is squashs a sum type all of whose 
constructors take the same argument by removing the constructors.

In order to understand more deeply the categorical relationship
between products and sums, we will consider the notion of
a heterogenous array containing strings and integers.
Of course the array has to be homogenous so we do this:
...
Now we can index the array with values of the sum type 3,
which is the sum of three units, and the result is a U,
which can then be decoded. However it is possible to
directly index a heterogenous array:
...
but the results have different types. We can fix this by unifying
them:
...
More generally, any tuple can be dynamically indexed by using
a generalised projection which returns the dual of the tuple
type:
...
where the RHS is an anonymous sum with numbered constructors.
In Felix the notation is ugly, the results of the projection
in order would be:
...
Now the point is that if all the sums have the same argument type,
we can squash the constructor away, and that is precisely
what happens with ordinary array indexing. In particular,
this is a special case because for an arbitrary tuple
you can't squash the constructor. The key is that the
core indexing operation applies to any product and returns
a value of the dual sum type.

The important thing here is that the fields of a product and
constructors of a sum are perfectly dualised and indexable,
the special case of arrays is handled using N-ary versions
of $\Delta$ and $\nabla$ to introduce and eliminate indexes.

The categorical relations here are crucial for understanding
the translations of these operations to the temporal domain. We begin
to picture that the decoder chip is a real sum, which is used
to lift the functional sum type into a temporal one.
The functional sum is a cheat, the real sum is the temporal one.

With products, we actually have a drop instead of a lift. 
The functional product is a spatial one, and the temporal
version of it feels unnatural. The reason is that what we
really want for a temporal product is a sequence of data
going down a channel, as for a list source.

In fact, you can write any values you want down channels,
they do not have to be the same type. The result is not
ill-typed, provided the reader expects the values to have
the correct type.

In the temporal domain we do not need to wrap the 
values in constructors to unify them, we just need to check
that the type sequence the reader and writer use agree.
We don't have a type checker that can do that at the moment!
