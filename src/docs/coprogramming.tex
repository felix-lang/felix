...
\begin{minted}{felix}
// UNTESTED: TESTME
chip longest (a: list[iochip_t[Buffer,Buffer]])
  connector io
    pin inp: %<Buffer
    pin out: %>Buffer
{
  var x = read io.inp;
  var wi,wo = mk_ioschannel_pair[Buffer];
  var results = Empty[Buffer];
  var processor = pipeline_list ([number,drop &results]);
  circuit
     wire wi to processor.inp;
  endcircuit
  run { write (wo, x); };

  // find longest match, if any
  match results with
  | Empty => ; // no match do nothing
  | head ! tail =>
     var big = head;
     for elt in tail do 
       if elt > big do big = elt; done
     done
     write (io.out, big);
   endmatch;
}
...
\index[codeindex]{gramentry\_t}
\index[codeindex]{gramlib\_t}
\index[codeindex]{grammar\_t}
\begin{minted}{felix}
...
\index[codeindex]{make\_recogniser\_from\_grammar}
\begin{minted}{felix}
