...
// if a valid empty array (null pointer), so create an array of population == 1:
        ...
        if (Pjlw == (Pjlw_t)NULL)
        {
            ...
            //fprintf(stderr, "Updating gc node because: valid empty array (%p)\n", (Pvoid_t)Pjlwnew);
...
//fprintf(stderr, "JU_INSERTCOPY offset =%d\n",offset);
                ...
                //fprintf(stderr, "Updating gc node because: growing to a larger leaf (%p)\n", (Pvoid_t)Pjlwnew );
                *PPArray = (Pvoid_t) Pjlwnew;
                DBGCODE(JudyCheckPop(*PPArray);)
...
// counted in a JPM at all:
            ...
            //fprintf(stderr, "Updating gc node because: leaf is at max size (%p)\n", (Pvoid_t)Pjpm );
